...
public sealed class BackgroundTaskStepHandler<T> where T : class, IProcessableEntity
{
    ...
    public Task HandleAsync(IProcessableEntityStep step, IEnumerable<T> data, CancellationToken cToken) => _handlers.ContainsKey(step.Id)
    ...
    public Task<IReadOnlyCollection<T>> HandleAsync(IProcessableEntityStep step, CancellationToken cToken) => _handlers.ContainsKey(step.Id)
