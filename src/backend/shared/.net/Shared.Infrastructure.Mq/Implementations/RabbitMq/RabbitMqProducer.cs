...
internal sealed class RabbitMqProducer : IMqProducer
{
    ...
    public void Dispose()
    {
        ...
        _logger.LogDebug(_initiator, Constants.Disconnect, Constants.Success);
    }
    ...
    private void Publish(RabbitMqProducerMessage message)
    {
        ...
        _logger.LogTrace(_initiator, Constants.Post, Constants.Start, message.Id);
        ...
        _model.BasicPublish(
            ...
            , Encoding.UTF8.GetBytes(JsonExtensions.SerializeToString(message.Payload)));

        _logger.LogTrace(_initiator, Constants.Post, Constants.Success, message.Id);
    }
