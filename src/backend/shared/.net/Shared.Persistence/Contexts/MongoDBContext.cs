...
public abstract class MongoDBContext
{
    ...
    public async Task CreateAsync<T>(T entity) where T : class, IPersistent
    {
        var collection = GetCollection<T>();
        await collection.InsertOneAsync(entity);
    }
    public async Task CreateAsync<T>(T entity, CancellationToken cToken) where T : class, IPersistent
    {
        var collection = GetCollection<T>();
        await collection.InsertOneAsync(entity, null, cToken);
    }
    public async Task CreateRangeAsync<T>(IReadOnlyCollection<T> entities) where T : class, IPersistent
    {
        var collection = GetCollection<T>();
        await collection.InsertManyAsync(entities);
    }
    public async Task CreateRangeAsync<T>(IReadOnlyCollection<T> entities, CancellationToken cToken) where T : class, IPersistent
    {
        var collection = GetCollection<T>();
        await collection.InsertManyAsync(entities, null, cToken);
    }

    public Task UpdateAsync<T>(object[] id, T entity) where T : class, IPersistent
    {
