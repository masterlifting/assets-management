...
namespace AM.Services.Portfolio.Core.Services.EntityStateService.Steps.Parsing.ReportFiles.Bcs;
...
public sealed class BcsReportParser
{
    ...
    public async Task StartAsync(IEnumerable<ReportFile> files, CancellationToken cToken)
    {
    ...
    private Task<BcsReport?[]> GetBcsReportsAsync(IEnumerable<ReportFile> files, IDictionary<string, string[]> derivativeDictionary, CancellationToken cToken)
        => Task.WhenAll(files
                     ...
                     .GroupBy(x => x!.Body!.AccountId))
        {
            ...
            foreach (var bcsReport in reportsGroup)
            {
                var report = new Report
                {
                    ...
                    ReportFileId = bcsReport.File.Id
                };
            ...
            {
                foreach (var bcsReport in reportsGroup.Join(currentReports, x => x!.File.Id, y => y.ReportFileId, (x,_) => x))
                {
                    bcsReport!.File.StateId = (int)States.Error;
                    bcsReport.File.Info = exception.Message;
                }
