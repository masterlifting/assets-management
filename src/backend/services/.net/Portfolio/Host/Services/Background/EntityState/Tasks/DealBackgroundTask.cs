...
{
    public sealed class DealBackgroundTask : IEntityStateBackgroundTask
    {
        ...
        public async Task StartAsync(int count, BackgroundTaskSettings settings, CancellationToken cToken)
        {
            ...
            var pipeline = serviceProvider.GetRequiredService<EntityStateHandling<Deal, IDealRepository>>();
