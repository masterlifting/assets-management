...
using Shared.MQ.Exceptions;
using Shared.MQ.Settings.RabbitMq;

using System.Text;

namespace Shared.MQ.Domain.RabbitMq.Connection;
...
public sealed class RabbitMqProducer : IMqProducer
{
    ...
    public bool TryPublish<TPayload>(IMqProducerMessage<TPayload> message, out string error) where TPayload : class
    {
        ...
        {
            _logger.LogError(new SharedMQException(nameof(RabbitMqProducer), Constants.Actions.Post, new(exception)));
